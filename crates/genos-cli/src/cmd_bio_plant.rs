...
use genos_core::biomimicry::{AbscissionProcess, SeedDormancy};
...
pub fn plant_seed(params: &[String]) -> Result<()> {
    ...
    if action == "pack" {
        ...
        seed.germination_condition = param_value(params, "condition")
            .unwrap_or("cpu_idle")
            .to_string();
...
pub fn plant_abscission(params: &[String]) -> Result<()> {
    let swarm_id = param_value(params, "swarm_id")
        .unwrap_or("swarm_alpha")
        .to_string();
    let target = param_value(params, "target_module")
        .ok_or_else(|| anyhow::anyhow!("missing target_module"))?;
    let reclaim: u64 = param_value(params, "reclaim_budget")
        .unwrap_or("150")
        .parse()?;
