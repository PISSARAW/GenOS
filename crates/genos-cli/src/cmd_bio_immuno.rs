...
use genos_core::biomimicry::{InflammatoryResponse, AutoImmunityRegulator};
...
pub fn immuno_inflammation(params: &[String]) -> Result<()> {
    let swarm_id = param_value(params, "swarm_id").unwrap_or("global_swarm").to_string();
    ...
    if action == "trigger" {
        let threat: f64 = param_value(params, "threat_level").unwrap_or("0.5").parse()?;
        response.trigger_response(threat);
        println!("Inflammation evaluated for Swarm {}. Severity: {:.2}, State: {:?}", swarm_id, response.severity, response.state);
        if response.is_quarantined() {
            println!("QUARANTINE ENFORCED: Degraded mode active.");
        }
    } else if action == "resolve" {
        let rate: f64 = param_value(params, "recovery_rate").unwrap_or("0.2").parse()?;
        // Mocking severity for resolution
        response.severity = 0.9; 
        response.resolve_over_time(rate);
        println!("Inflammation cooling down. New severity: {:.2}, State: {:?}", response.severity, response.state);
    } else {
...
pub fn immuno_autoimmunity(params: &[String]) -> Result<()> {
    ...
    if action == "log_kill" {
        ...
        println!("Defensive kill logged. Total recent kills: {}", regulator.recent_defensive_kills);
    } else if action == "evaluate" {
        // Mock kills
        regulator.recent_defensive_kills = param_value(params, "recent_kills").unwrap_or("4").parse()?;
