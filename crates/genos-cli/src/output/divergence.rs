...
pub fn print_divergence_handoff(report: &DivergenceReport) {
    ...
    println!("    à l'étape {}, mais j'ai identifié un point de restauration sécurisé (LKGS).\"", report.causal_divergence_step);
    println!();
    ...
    println!("     $ genos fork --from-step {}", report.safest_revert_step);
    println!();
    println!("   Option B - Reprendre la main manuellement depuis la divergence :");
    println!("     $ genos agent resume --manual --step {}", report.causal_divergence_step);
    println!();
