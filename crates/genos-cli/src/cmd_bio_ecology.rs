...
use genos_core::biomimicry::{
    EcologicalSuccession, EvolutionPhase, PunctuatedEquilibria, SuccessionStage,
};
...
pub fn ecology_punctuated(params: &[String]) -> Result<()> {
    let agent_id = param_value(params, "agent_id")
        ...
        .to_string();
    let threshold: usize = param_value(params, "plateau_threshold")
        .unwrap_or("5")
        .parse()?;
    ...
    let mut stasis_counter: usize = param_value(params, "stasis_counter")
        .unwrap_or("0")
        .parse()?;
    ...
    if new_phase == EvolutionPhase::Punctuation {
        println!(
            "PLATEAU DETECTED! Triggering evolutionary burst (high temperature / high mutation)."
        );
    } else {
        println!(
            "Stasis Phase. Incremental refinement. Counter: {}/{}",
            pe.stasis_counter, pe.plateau_threshold
        );
    }
...
pub fn ecology_succession(params: &[String]) -> Result<()> {
    let project_id = param_value(params, "project_id")
        .unwrap_or("project_x")
        .to_string();
    ...
    match new_stage {
        ...
        SuccessionStage::Intermediate => {
            println!("Intermediate Stage. Spawn builders and structural agents.")
        }
        SuccessionStage::Climax => {
            println!("Climax Stage. Spawn expensive, specialized polishers.")
        }
    }
