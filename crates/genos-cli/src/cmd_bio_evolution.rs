...
pub fn speciation_check(params: &[String]) -> Result<()> {
    let alleles_a: Vec<String> = collect_params(params, "allele-a")
        .iter()
        .map(|s| s.to_string())
        .collect();
    let alleles_b: Vec<String> = collect_params(params, "allele-b")
        .iter()
        .map(|s| s.to_string())
        .collect();
...
pub fn bet_hedge_allocate(params: &[String]) -> Result<()> {
    ...
    let total_budget: u64 = param_value(params, "total_budget")
        ...
        .parse()?;
    let entropy: f64 = param_value(params, "entropy")
        .and_then(|v| v.parse().ok())
        .unwrap_or(0.3);
    ...
    println!(
        "Insurance fraction: {:.1}%",
        plan.insurance_fraction * 100.0
    );
    Ok(())
}
