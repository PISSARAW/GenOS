...
use genos_core::biomimicry::{EndosymbiosisEngine, OrganelleStatus, BloodBrainBarrier};
...
pub fn cellular_endosymbiosis(params: &[String]) -> Result<()> {
    ...
    let external_tool = param_value(params, "external_tool").ok_or_else(|| anyhow::anyhow!("missing external_tool"))?.to_string();
    let success_rate: f64 = param_value(params, "success_rate").unwrap_or("0.9").parse()?;
