//! Apprentissage du directeur (enregistrement, plasticité, crédit).
//!
...
use crate::learning::{CreditAssignment, LearnerUpdate};
...
impl Director {
    ...
    pub fn record(&mut self, concept: Concept, success: bool) {
        ...
        if !self.last_context.is_empty() {
            ...
            self.learner.update(LearnerUpdate {
                concept,
                ctx: &context,
    ...
    pub fn assign_credit(&mut self, plan: &[Concept], reward: f64) {
        ...
        let context = self.last_context.clone();
        self.learner.assign_credit(CreditAssignment { plan, ctx: &context, reward });
    }
