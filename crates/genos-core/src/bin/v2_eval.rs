...
fn main() {
    ...
    let reflection = v2.mind.as_mut().unwrap().recurrence.deliberate_complex_problem("Qui suis-je et quelle est mon architecture ?", true);
    println!("{}\n", reflection);
    ...
    match v2.mind.as_mut().unwrap().causality.deliberate_causality("Auto-introspection du code source", "Compréhension de soi acquise") {
        Ok(res) => println!("{}\n", res),
        Err(e) => println!("{}\n", e),
    }
    ...
    let ood = v2.mind.as_mut().unwrap().ood_resilience.handle_reality_shift(
    ...
    match v2.mind.as_mut().unwrap().semantic_grounding.validate_communication(
    ...
    match v2.mind.as_mut().unwrap().halting.should_halt(800, 0.9, 0.95, true) {
        Ok(res) => println!("{}\n", res),
        Err(e) => println!("{}\n", e),
    }
