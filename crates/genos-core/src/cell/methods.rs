...
impl AgentCell {
    ...
    pub fn trigger_apoptosis(&mut self) {
        self.lifecycle_state = crate::cell::LifecycleState::Apoptotic(crate::cell::lifecycle::ApoptoticState::default());
    ...
    pub fn translate_mrna(&mut self, mrna_sequence: Vec<String>) -> Result<(), String> {
        ...
        if let Some(mind) = self.mind_mut() {
            for (i, codon) in mrna_sequence.iter().enumerate() {
                ...
                mind.trace.sequence.push(crate::cell::events::CellEvent::TaskExecuted { 
                    task_name: "Synthèse".to_string(), 
                    result: codon.clone() 
                });
            }
