...
pub fn evaluate_convergence(
    ...
    let trace_a: Vec<String> = agent_a.mind().unwrap().trace.sequence.iter().map(|e| format!("{:?}", e)).collect();
    let trace_b: Vec<String> = agent_b.mind().unwrap().trace.sequence.iter().map(|e| format!("{:?}", e)).collect();
    let trace_distance = trace_levenshtein(&trace_a, &trace_b);
