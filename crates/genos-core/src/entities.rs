...
impl EntityRef {
    ...
    pub fn intersects(&self, other: &EntityRef) -> bool {
        match (self, other) {
            ...
            (
                EntityRef::DirectoryWildcard {
                    path: dir_path,
                    pattern,
                },
                EntityRef::File { path: p2 },
            )
            | (
                EntityRef::DirectoryWildcard {
                    path: dir_path,
...
mod tests {
    ...
    fn test_line_range_collision() {
        let block1 = EntityRef::LineRange {
            path: "main.rs".into(),
            start: 10,
            end: 20,
        };
        let block2 = EntityRef::LineRange {
            path: "main.rs".into(),
            start: 15,
            end: 25,
        }; // Overlap
        let block3 = EntityRef::LineRange {
            path: "main.rs".into(),
            start: 50,
            end: 60,
        }; // Indépendant
        let block4 = EntityRef::LineRange {
            path: "other.rs".into(),
            start: 15,
            end: 25,
        }; // Autre fichier
    ...
    fn test_wildcard_collision() {
        let grep = EntityRef::DirectoryWildcard {
            path: "src/".into(),
            pattern: "*.rs".into(),
        };
        let src_file = EntityRef::File {
            path: "src/main.rs".into(),
        };
        let css_file = EntityRef::File {
            path: "src/style.css".into(),
        };
