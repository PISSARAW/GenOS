...
pub struct PheromoneDeposit<'a> {
    pub node_id: &'a str,
    pub pheromone_type: PheromoneType,
    pub amount: f64,
}

pub struct OsmoticRoute<'a> {
    pub source: &'a str,
    pub destination: &'a str,
    pub pheromone_type: &'a PheromoneType,
}
...
impl SpatialMesh {
    ...
    pub fn add_edge(&self, node_a: &str, node_b: &str) {
        ...
        map.entry(node_a.to_string())
            .or_default()
            .push(node_b.to_string());
        map.entry(node_b.to_string())
            .or_default()
            .push(node_a.to_string());
    }
    ...
    pub fn deposit(&self, deposit: PheromoneDeposit<'_>) {
        let mut map = self.gradients.write().unwrap();
        let node = map.entry(deposit.node_id.to_string()).or_default();
        *node.entry(deposit.pheromone_type).or_insert(0.0) += deposit.amount;
    }
    ...
    pub fn osmotic_flow(&self, route: OsmoticRoute<'_>) -> f64 {
        let grad_src = self.sense(route.source, route.pheromone_type);
        let grad_dst = self.sense(route.destination, route.pheromone_type);
