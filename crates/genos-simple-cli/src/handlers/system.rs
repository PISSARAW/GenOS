...
async fn handle_system_async(cmd: &SystemCommands, yes: bool) {
    match cmd {
        ...
        SystemCommands::Generate { args } => {
            ...
            if let Ok(res) = crate::apply_api_auth(client.post(format!("{}/v1/chat/completions", api_url))).json(&body2).send().await {
                ...
                if let Ok(json_resp) = res.json::<serde_json::Value>().await { if let Some(text) = json_resp["choices"][0]["message"]["content"].as_str() { let clean = text.trim().strip_prefix("```json").unwrap_or(text.trim()).strip_suffix("```").unwrap_or(text.trim()); if let Ok(files) = serde_json::from_str::<serde_json::Value>(clean) { if let Some(file_array) = files.as_array() { for file in file_array { if let (Some(name), Some(content)) = (file["filename"].as_str(), file["content"].as_str()) { let fp = target_dir.join(name); let canonical = target_dir.canonicalize().unwrap_or(target_dir.clone()); if let Some(parent) = fp.parent() { std::fs::create_dir_all(parent).unwrap_or_else(|e| command_error(format!("impossible de créer le parent du fichier: {}", e))); } if std::fs::write(&fp, content).is_ok() { println!("✔️ Créé : {}", fp.display()); } } } } else { println!("⚠️ Le modèle n'a pas respecté le format JSON strict."); } } } }
            }
