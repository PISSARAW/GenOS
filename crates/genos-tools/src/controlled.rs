...
impl BrowserTool {
    pub fn new(policy: BrowserPolicy) -> Result<Self> {
        Ok(Self {
    ...
    pub async fn fetch_text(&self, address: &str) -> Result<String> {
        ...
        if !self.policy.allows(&url) {
            bail!("browser policy forbids URL host or scheme");
        }
        let response = self.client.get(url).send().await?.error_for_status()?;
        let bytes = response.bytes().await?;
        if bytes.len() > self.policy.max_response_bytes {
            bail!("browser response exceeds policy size limit");
        }
...
impl PythonInterpreter {
    pub fn new(policy: PythonPolicy) -> Self {
        Self { policy }
    }
    pub async fn execute(&self, source: &str) -> Result<PythonResult> {
        if !self.policy.enabled {
            bail!("Python execution is disabled by policy");
        }
        if !self.policy.working_dir.is_dir() {
            bail!("Python working directory does not exist");
        }
        let child = Command::new("python3")
            ...
            .output();
        let output = timeout(self.policy.timeout, child)
            .await
            .map_err(|_| anyhow::anyhow!("Python execution timed out"))??;
        if output.stdout.len() + output.stderr.len() > self.policy.max_output_bytes {
            bail!("Python output exceeds policy size limit");
        }
        Ok(PythonResult {
...
mod tests {
    ...
    fn browser_policy_only_allows_declared_hosts() {
        let policy = BrowserPolicy {
            allowed_hosts: BTreeSet::from(["docs.example.test".into()]),
    ...
    async fn python_interpreter_obeys_enable_policy() {
        let interpreter = PythonInterpreter::new(PythonPolicy {
