...
fn test_dominant_recessive() {
    let locus_a = genos_core::Locus { gene_name: "trait".to_string(), value: 0.2, epigenetic_marker: 0.1 };
    let locus_b = genos_core::Locus { gene_name: "trait".to_string(), value: 0.8, epigenetic_marker: 0.0 };
    let mut prng = 42;

    let res = super::breeding::calculate_recombined_locus(&locus_a, &locus_b, false, &genos_core::RecombinationStrategy::DominantRecessive, &mut prng);
    assert_eq!(res.value, 0.8);
}

#[test]
fn test_epistatic_masking() {
    let locus_a = genos_core::Locus { gene_name: "trait".to_string(), value: 0.2, epigenetic_marker: 0.9 };
    let locus_b = genos_core::Locus { gene_name: "trait".to_string(), value: 0.8, epigenetic_marker: 0.1 };
    let mut prng = 42;

    let res = super::breeding::calculate_recombined_locus(&locus_a, &locus_b, false, &genos_core::RecombinationStrategy::Epistatic, &mut prng);
    assert_eq!(res.value, 0.2);
}

#[test]
fn test_gaussian_recombination() {
    let locus_a = genos_core::Locus { gene_name: "trait".to_string(), value: 0.2, epigenetic_marker: 0.0 };
    let locus_b = genos_core::Locus { gene_name: "trait".to_string(), value: 0.8, epigenetic_marker: 0.0 };
    let mut prng = 42;

    let res = super::breeding::calculate_recombined_locus(&locus_a, &locus_b, false, &genos_core::RecombinationStrategy::Gaussian, &mut prng);
...
fn test_uniform_crossover() {
    ...
    let res = super::breeding::calculate_recombined_locus(&locus_a, &locus_b, false, &genos_core::RecombinationStrategy::UniformCrossover { mix_probability: 1.0 }, &mut prng);
    assert_eq!(res.value, 0.8);
    
    let res = super::breeding::calculate_recombined_locus(&locus_a, &locus_b, false, &genos_core::RecombinationStrategy::UniformCrossover { mix_probability: 0.0 }, &mut prng);
    assert_eq!(res.value, 0.2);
}
