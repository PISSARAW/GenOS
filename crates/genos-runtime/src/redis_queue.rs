...
impl RedisTaskQueue {
    pub fn new(url: &str, namespace: impl Into<String>) -> Result<Self> {
        ...
        anyhow::ensure!(
            ...
            "redis queue namespace is required"
        );
        Ok(Self {
    ...
    pub async fn enqueue<T: Serialize>(&self, payload: T) -> Result<String> {
        ...
        connection
            .lpush::<_, _, ()>(self.key("pending"), encoded)
            .await
            .context("enqueueing Redis task")?;
        Ok(id)
    }

    pub async fn claim<T: DeserializeOwned + Serialize>(
        &self,
        timeout: Duration,
    ) -> Result<Option<RedisClaim<T>>> {
        let mut connection = self.connection().await?;
    ...
    pub async fn acknowledge<T>(&self, claim: RedisClaim<T>) -> Result<()> {
        self.transition(&claim.raw, "completed").await
    }
    pub async fn fail<T>(&self, claim: RedisClaim<T>) -> Result<()> {
        self.transition(&claim.raw, "failed").await
    }
    ...
    async fn transition(&self, raw: &str, target: &str) -> Result<()> {
        ...
        let removed: i64 = script
            ...
            .await
            .context("transitioning Redis task")?;
    ...
    W: TaskWorker<T> + 'static,
{
    ...
    pub async fn run_once(&self) -> Result<bool> {
        let Some(claim) = self.queue.claim::<T>(self.poll_timeout).await? else {
            return Ok(false);
        };
