...
impl PersistentTaskQueue {
    ...
    pub async fn recover_expired<T: DeserializeOwned>(&self, lease_ms: u128) -> Result<usize> {
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if metadata
                ...
                .map(|age| age.as_millis() >= lease_ms)
                .unwrap_or(false)
            {
    ...
    pub async fn claim<T: DeserializeOwned + Serialize>(&self) -> Result<Option<DurableTask<T>>> {
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if fs::rename(entry.path(), &processing).await.is_err() {
                continue;
            }
            let mut task: DurableTask<T> = serde_json::from_slice(&fs::read(&processing).await?)
                .with_context(|| format!("decoding task {}", name.to_string_lossy()))?;
            task.attempts += 1;
    ...
    pub async fn complete(&self, id: &str) -> Result<()> {
        self.move_task(id, "completed").await
    }
    pub async fn fail(&self, id: &str) -> Result<()> {
        self.move_task(id, "failed").await
    }
...
{
    pub fn new(queue: PersistentTaskQueue, worker: Arc<W>, poll_interval: Duration) -> Self {
        Self {
            queue,
            worker,
            poll_interval,
    ...
    pub async fn run_once(&self) -> Result<bool> {
        let Some(task) = self.queue.claim().await? else {
            return Ok(false);
        };
        let id = task.id.clone();
        match self.worker.process(task).await {
            Ok(()) => self.queue.complete(&id).await?,
            Err(_) => self.queue.fail(&id).await?,
        }
        Ok(true)
    }
...
mod tests {
    ...
    struct Worker;
    #[async_trait]
    impl TaskWorker<String> for Worker {
        async fn process(&self, task: DurableTask<String>) -> Result<()> {
            anyhow::ensure!(task.payload == "ok");
            Ok(())
        }
    }
    #[tokio::test]
