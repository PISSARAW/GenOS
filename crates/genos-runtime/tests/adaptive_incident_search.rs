...
fn adaptive_search_refines_partial_reproductions_into_three_perfect_matches() {
    let report = run_incident_search(IncidentSearchManifest {
        ...
        evidence: IncidentEvidence {
            ...
            preceding_events: vec!["evt-901".to_string(), "evt-902".to_string(), "evt-903".to_string()],
        },
        inferred_crash_signature: IncidentMutation {
            timing_skew_ms: 73.0, network_latency_ms: 180.0, packet_loss_percent: 2.5,
            reorder_events: true, db_isolation: "serializable".to_string(), concurrency: 48,
            cache_eviction_ratio: 0.82,
        },
        config: IncidentSearchConfig {
            seed: 42, initial_universes: 100, partial_survivors: 11,
            descendants_per_survivor: 4, targeted_descendants: 3,
        },
    ...
    assert!(report.descendants.iter().filter(|result| {
        result.status == ReproductionStatus::PerfectlyReproduced
    }).all(|result| result.reproduction_score == 1.0));
    assert!(report.initial_universes.iter().chain(&report.descendants).all(|result| {
        result.replayed_event_ids == vec!["evt-901", "evt-902", "evt-903"]
    }));
}
