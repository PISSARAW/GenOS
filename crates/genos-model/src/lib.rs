...
fn validate_schema_at(value: &Value, schema: &Value, path: &str) -> anyhow::Result<()> {
    if let Some(expected) = schema.get("type").and_then(Value::as_str) {
        ...
        if !valid {
            anyhow::bail!("{path}: expected {expected}");
        }
    }
    if let Some(values) = schema.get("enum").and_then(Value::as_array) {
        if !values.iter().any(|candidate| candidate == value) {
            anyhow::bail!("{path}: value is not in enum");
        }
    }
    if let Some(object) = value.as_object() {
        if let Some(required) = schema.get("required").and_then(Value::as_array) {
            for field in required {
                let field = field
                    .as_str()
                    .ok_or_else(|| anyhow::anyhow!("{path}: required fields must be strings"))?;
                if !object.contains_key(field) {
                    anyhow::bail!("{path}.{field}: required field is missing");
                }
            }
        }
        ...
        if schema.get("additionalProperties").and_then(Value::as_bool) == Some(false) {
            if let Some(properties) = properties {
                if let Some(unknown) = object.keys().find(|key| !properties.contains_key(*key)) {
                    anyhow::bail!("{path}.{unknown}: additional property is not allowed");
                }
            }
        }
        if let Some(properties) = properties {
            for (key, child_schema) in properties {
                if let Some(child) = object.get(key) {
                    validate_schema_at(child, child_schema, &format!("{path}.{key}"))?;
                }
            }
        }
    }
    if let Some(items) = value.as_array() {
        if let Some(item_schema) = schema.get("items") {
            for (index, item) in items.iter().enumerate() {
                validate_schema_at(item, item_schema, &format!("{path}[{index}]"))?;
            }
        }
    }
    Ok(())
...
pub fn parse_structured_with_schema<T: serde::de::DeserializeOwned>(
    response: &LlmResponse,
    schema: &Value,
) -> anyhow::Result<T> {
    let content = response
...
mod schema_tests {
    ...
    fn validates_nested_contracts() {
        ...
        assert!(
            validate_json_schema(&serde_json::json!({"items":[{"name":"ok"}]}), &schema).is_ok()
        );
        assert!(validate_json_schema(&serde_json::json!({"items":[{}]}), &schema).is_err());
        assert!(
            validate_json_schema(&serde_json::json!({"items":[],"extra":true}), &schema).is_err()
        );
    }
}
