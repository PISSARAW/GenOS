...
async function main() {
  ...
  try {
    ...
    test('monadologie describes agent as monad', () => {
      const m = leibnizianService.monadologie({ agent: { id: 'agent-1' } });
      assert.strictEqual(m.agentId, 'agent-1');
      assert.strictEqual(m.substance, 'monade');
      assert.strictEqual(m.type, 'simple_substance');
      assert.strictEqual(m.indivisible, true);
      assert.strictEqual(m.sansParties, true);
      assert.strictEqual(m.refleteUniverse, true);
      assert.ok(m.description);
    });
    test('monadologie throws on missing agent', () => {
      assert.throws(() => leibnizianService.monadologie({}), /requires an agent/);
    });
    test('harmoniePreEtablie returns pre-established coordination', () => {
      const h = leibnizianService.harmoniePreEtablie({ agent: { id: 'agent-1' }, schedule: ['task-a', 'task-b'] });
      assert.strictEqual(h.agentId, 'agent-1');
      ...
      assert.deepStrictEqual(h.programme, ['task-a', 'task-b']);
    });
    test('harmoniePreEtablie requires agent', () => {
      assert.throws(() => leibnizianService.harmoniePreEtablie({ schedule: [] }), /requires an agent/);
    });
    test('principeRaisonSuffisante validates action with reason', () => {
      const r = leibnizianService.principeRaisonSuffisante({ action: 'compute', reason: 'needed for optimization' });
      assert.strictEqual(r.action, 'compute');
      assert.strictEqual(r.hasReason, true);
      assert.strictEqual(r.valid, true);
      assert.strictEqual(r.principle, 'nihil est sine ratione');
    });
    test('principeRaisonSuffisante rejects action without reason', () => {
      const r = leibnizianService.principeRaisonSuffisante({ action: 'compute' });
      assert.strictEqual(r.hasReason, false);
      assert.strictEqual(r.valid, false);
      assert.strictEqual(r.principle, 'nihil est sine ratione');
    });
    test('loisDeContinuation detects continuous series', () => {
      const events = [
        { timestamp: 1000 },
        { timestamp: 1500 },
        { timestamp: 2000 },
      ];
      const result = leibnizianService.loisDeContinuation({ events });
      assert.strictEqual(result.continuous, true);
      assert.strictEqual(result.gaps, 0);
      assert.strictEqual(result.law, 'natura non facit saltus');
    });
    test('loisDeContinuation detects discontinuities', () => {
      const events = [
        { timestamp: 1000 },
        { timestamp: 5000 },
      ];
      ...
      assert.strictEqual(result.gaps, 1);
    });
    test('loisDeContinuation requires events array', () => {
      assert.throws(() => leibnizianService.loisDeContinuation({}), /requires events array/);
    });
    test('calculRaisonSuffisante finds reason with valid causes', () => {
      const result = leibnizianService.calculRaisonSuffisante({ state: 'running', causes: [{ explanation: 'initialized' }, { explanation: 'started' }] });
      assert.strictEqual(result.state, 'running');
      assert.strictEqual(result.reasonFound, true);
      assert.ok(result.bestCause);
      assert.strictEqual(result.bestCause.explanation, 'initialized');
      assert.strictEqual(result.principle, 'raison suffisante');
    });
    test('calculRaisonSuffisante finds no reason without causes', () => {
      const result = leibnizianService.calculRaisonSuffisante({ state: 'unknown' });
      assert.strictEqual(result.reasonFound, false);
      assert.strictEqual(result.bestCause, null);
    });
