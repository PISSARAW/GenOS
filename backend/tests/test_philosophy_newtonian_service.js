...
async function main() {
  ...
  try {
    ...
    test('espaceAbsolu describes absolute space', () => {
      const s = newtonianService.espaceAbsolu({ system: { agents: 3 } });
      assert.strictEqual(s.framework, 'newtonian');
      assert.strictEqual(s.absolute, true);
      assert.strictEqual(s.dimensions, 3);
      assert.strictEqual(s.homogeneous, true);
      assert.ok(s.description);
    });
    test('espaceAbsolu works without system', () => {
      const s = newtonianService.espaceAbsolu({ system: {} });
      assert.strictEqual(s.framework, 'newtonian');
      assert.deepStrictEqual(s.system, {});
    });
    test('tempsAbsolu describes absolute time', () => {
      const t = newtonianService.tempsAbsolu({ system: { clock: 'utc' } });
      assert.strictEqual(t.framework, 'newtonian');
      assert.strictEqual(t.absolute, true);
      assert.strictEqual(t.direction, 'uniform');
      assert.strictEqual(t.independentOfMotion, true);
    });
    test('mecaniqueClassique computes action-reaction', () => {
      const f = newtonianService.mecaniqueClassique({ agent1: { id: 'a1' }, agent2: { id: 'a2' }, force: 10 });
      assert.strictEqual(f.law, 'action-reaction');
      assert.strictEqual(f.agent1.id, 'a1');
      assert.strictEqual(f.agent2.id, 'a2');
      assert.strictEqual(f.forceOnAgent1, 10);
      assert.strictEqual(f.forceOnAgent2, -10);
      assert.strictEqual(f.inertia, true);
    });
    test('mecaniqueClassique defaults force to 0', () => {
      const f = newtonianService.mecaniqueClassique({ agent1: { id: 'a1' }, agent2: { id: 'a2' } });
      assert.strictEqual(f.forceOnAgent1, 0);
      assert.strictEqual(f.forceOnAgent2, -0);
    });
    test('mecaniqueClassique throws on missing agent', () => {
      assert.throws(() => newtonianService.mecaniqueClassique({ agent1: { id: 'a1' } }), /requires agent1 and agent2/);
    });
