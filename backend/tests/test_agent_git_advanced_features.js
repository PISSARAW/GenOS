...
dbModule.getDatabase = async () => ({
  ...
  all: async (sql) => sql.includes('agent_git_objects') ? [{ id: 'one', agent_id: 'agent-1', object_kind: 'commit', ref_name: 'main', remote_name: null, state_hash: 'hash-one', state_json: JSON.stringify(state), metadata_json: JSON.stringify({ locked: false }), signature: 'sig-one', created_at: '2026-01-01' }] : [],
  run: async (sql, ...args) => { writes.push({ sql, args }); return { changes: 1 }; }
});

// Clear all module caches to pick up latest changes
for (const key of Object.keys(require.cache)) {
  if (key.includes('agentGitService') || key.includes('canonical') || key.includes('commitGraph') || key.includes('storeObjectHelper')) {
...
(async () => {
  ...
  assert.equal(log.objects.length, 1);
  const replay = await service.replay(req);
  assert.equal(replay.runtimeReplay.eventCount, 1);
