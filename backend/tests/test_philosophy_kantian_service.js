...
async function main() {
  ...
  try {
    ...
    test('phenomene describes observable experience', () => {
      const p = kantianService.phenomene({ agent: { id: 'agent-1' }, observation: 'red apple' });
      assert.strictEqual(p.agentId, 'agent-1');
      assert.strictEqual(p.type, 'phénomène');
      assert.strictEqual(p.observable, true);
      assert.strictEqual(p.experience, 'red apple');
      assert.deepStrictEqual(p.categories, ['espace', 'temps', 'causalité', 'unité', 'pluralité']);
      assert.ok(p.description);
    });
    test('phenomene requires agent', () => {
      assert.throws(() => kantianService.phenomene({ observation: 'x' }), /requires an agent/);
    });
    test('noumene describes thing-in-itself', () => {
      const n = kantianService.noumene({ chose: 'noumenal reality' });
      assert.strictEqual(n.chose, 'noumenal reality');
      assert.strictEqual(n.type, 'noumène');
      assert.strictEqual(n.observable, false);
      assert.strictEqual(n.accessible, false);
      assert.ok(n.description);
    });
    test('noumene requires chose', () => {
      assert.throws(() => kantianService.noumene({}), /requires a chose/);
    });
    test('categoriesAPriori returns all categories', () => {
      const c = kantianService.categoriesAPriori();
      assert.strictEqual(c.type, 'a priori');
      assert.ok(c.categories.includes('espace'));
      assert.ok(c.categories.includes('temps'));
      assert.ok(c.categories.includes('causalité'));
      assert.ok(c.categories.includes('unité'));
      assert.ok(c.categories.includes('pluralité'));
      assert.ok(c.categories.includes('totalité'));
      assert.ok(c.description);
    });
    test('critiqueRaisonPure maps knowable vs unknowable', () => {
      const c = kantianService.critiqueRaisonPure({ agent: { id: 'agent-1' } });
      assert.strictEqual(c.agentId, 'agent-1');
      assert.ok(c.connaissable.includes('phénomène'));
      assert.ok(c.connaissable.includes('catégories a priori'));
      assert.ok(c.inconnaissable.includes('noumène'));
      assert.ok(c.inconnaissable.includes('chose-en-soi'));
      assert.ok(c.limites);
    });
    test('critiqueRaisonPure requires agent', () => {
      assert.throws(() => kantianService.critiqueRaisonPure({}), /requires an agent/);
    });
    test('choseEnSoi describes subjective representation', () => {
      const c = kantianService.choseEnSoi({ agent: { id: 'agent-1' }, representation: 'the thing as perceived' });
      assert.strictEqual(c.agentId, 'agent-1');
      assert.strictEqual(c.choseEnSoi, 'the thing as perceived');
      assert.strictEqual(c.representation, 'représentation subjective');
      assert.strictEqual(c.limitesLesConnaissances, true);
    });
    test('choseEnSoi requires agent', () => {
      assert.throws(() => kantianService.choseEnSoi({ representation: 'x' }), /requires an agent/);
    });
    await testChain;
  } finally {
    await closeDatabase();
