...
async function main() {
  ...
  // This assertion is actually awaited (previously a floating .then()).
  const hostEmbedding = await symbioteRuntime.embedForSymbiote('host_orchestrator', 'text');
  assert.equal(hostEmbedding.engine, 'cloud');
  assert.equal(hostEmbedding.embedding, null);

  // Empty input is explicitly skipped, distinct from a runtime failure.
  const emptyEmbedding = await symbioteRuntime.embedForSymbiote('specialist_symbiont', '   ');
  ...
  const previousUrl = process.env.GENOS_EMBEDDING_URL;
  process.env.GENOS_EMBEDDING_URL = 'https://example.com';
  try {
    const invalidEndpoint = await symbioteRuntime.embedForSymbiote('specialist_symbiont', 'hello');
    assert.equal(invalidEndpoint.engine, 'local');
...
main().catch((error) => {
  console.error('SymbioteRuntime test failed:', error);
  process.exit(1);
});
