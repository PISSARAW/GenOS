...
async function main() {
  const session = await syncytium.createSession('Budget and release invariants.', {
    schema: {
      ...
      invariants: [
        ...
        { id: 'release-note-advisory', scope: ['release.note'], predicate: { op: 'present', path: 'release.note' }, severity: 'WARNING' }
      ]
    }
  });
  ...
  assert.equal(accepted.invariants.find((item) => item.invariantId === 'release-note-advisory').passed, false);
