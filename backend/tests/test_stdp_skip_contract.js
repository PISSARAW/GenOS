...
memory.stdpUpdate({ agentId: 'missing-pair' })
  .then((result) => { assert.equal(result.success, true); assert.equal(result.skipped, true); assert.equal(result.status, 'skipped'); console.log('STDP skip contract checks passed.'); })
