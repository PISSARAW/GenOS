...
async function runTests() {
  ...
  const result = await H.epistemicHolobionte(antigen, {
    domain: 'auth',
    stakes: 'normal',
    hostVeto: true,
    immuneMemory: [],
  ...
  const immuneReview2 = await H.immuneSymbiontReview(selfVerifiedAntigen, {});
  ...
  // Invariant 2: Le T-reg ne peut jamais inhiber un danger signal.
  ...
  // Invariant 3: La mémoire immunitaire — deux antigènes distincts ne collisionnent pas.
  ...
  assert.notStrictEqual(sigA, sigB, 'Deux antigènes distincts → signatures différentes');

  // Invariant 4: Un même antigène produit une même signature.
