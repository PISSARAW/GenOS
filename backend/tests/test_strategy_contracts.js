...
process.env.GENOS_PROMOTION_SECRET = process.env.GENOS_PROMOTION_SECRET || 'strategy-contract-test-secret';
...
function signedApproval(runId) {
  const timestamp = Date.now();
  const signerId = 'strategy-contract-reviewer';
  return {
    timestamp,
    signerId,
    signature: signatureService.generateSignature({ runId, timestamp, signerId }),
    ...
    replayReceipt: { success: true, status: 'verified' },
...
async function run() {
  ...
  const registryTotal = listStrategies().length;
  ...
  assert.equal(incident.strategy_decisions.length, registryTotal);
  assert.equal(incident.strategy_decision_summary.total_registry, registryTotal);
  ...
  assert.equal(research.decisions.length, registryTotal);
  ...
  try {
    ...
    assert.equal(registry.body.total, registryTotal);
    assert.equal(registry.body.registryTotal, registryTotal);
    ...
    assert.equal(preview.body.strategy_decisions.length, registryTotal);
    ...
    assert.equal(upgraded.body.contract.strategy_decisions.length, registryTotal);
