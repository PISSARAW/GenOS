...
class ProofArtifact {
  ...
  attachFormalResult(formalResult, formalization = null) {
    ...
    // formalStatementFingerprint as the reference for what Lean must prove.
    ...
    this._formalizationFp =
      formalization?.formalStatementFingerprint ?? null;
  ...
  async verifyThroughLean(leanGate, source) {
    ...
    // Binding check is the FIRST gate — formal authority takes precedence.
    if (this._formalization) {
      ...
      // Additional guard: reject any source that contains the statement as a
      // quoted string (e.g. theorem foo : "∀ n : Nat, n + 0 = n" := ...)
      // since checkSourceBinding may match a quoted form if the regex is loose.
      const hasQuotedStatement = /:\s*"[^"]*"/u.test(source);
      if (hasQuotedStatement && bindingCheck.headerStatement !== this._formalization.formalStatement) {
        throw new Error('Lean source wraps the statement in a string literal. The theorem header must contain the actual proposition, not a quoted string.');
      }
    ...
    // Secondary defense: reject trivial True proofs and placeholders AFTER binding.
    // These are regex-based guards — the formal binding check is authoritative.
