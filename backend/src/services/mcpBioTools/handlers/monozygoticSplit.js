...
function generateTwinClones(options) {
  const { clusterId, parentGenomeId, snapshotId, cloneCount, explorationSeeds, lineageId } = options;
  const clones = [];
  const dnaHash = crypto.createHash('sha256').update(parentGenomeId + snapshotId).digest('hex');

  for (let i = 0; i < cloneCount; i++) {
    const seed = explorationSeeds[i % explorationSeeds.length] || (i + 1) * 101;
    const cloneHash = crypto.createHash('sha256').update(clusterId + i).digest('hex').slice(0, 6);
    clones.push({
      cloneId: `twin-clone-${i + 1}-${cloneHash}`,
      parentGenomeId,
      genomeDnaHash: dnaHash,
      lineageId,
      generation: 2,
      seed,
      temperature: 0.2 + (i * 0.3),
...
function cleaveTwins(cluster, clusterId, args) {
  ...
  cluster.lineageId = `lin-mono-${crypto.createHash('sha256').update(parentGenomeId).digest('hex').slice(0, 6)}`;
  cluster.clones = generateTwinClones({
    clusterId, parentGenomeId, snapshotId, cloneCount, explorationSeeds, lineageId: cluster.lineageId
  });
...
function handleMonozygoticSplit(args = {}, run) {
  ...
  if (action === 'cleave_monozygotic_twins') return cleaveTwins(cluster, clusterId, args);
  if (action === 'synchronize_cleavage_state') return synchronizeCleavage(cluster, clusterId, args.trajectories);
