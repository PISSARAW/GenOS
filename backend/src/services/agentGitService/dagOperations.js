...
async function reset(req) {
  ...
  if (mode === 'hard') {
    ...
    await applyState(db, req, agentId, state, ['agent', 'decisions', 'memories', 'runs', 'plasmids', 'permissions']);
  }
...
function computePatch(fromState, toState) {
  ...
  for (const section of sections) {
    ...
    for (const item of toItems) {
      if (!fromIds.has(item.id)) operations.push({ op: 'ADD', section, item });
    }
    for (const item of fromItems) {
      if (!toIds.has(item.id)) operations.push({ op: 'REMOVE', section, itemId: item.id });
    }
  }
  return { operations };
}
