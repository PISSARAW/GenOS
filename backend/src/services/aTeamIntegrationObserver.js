...
function memberForWorker(worker, members, index) {
  return members.find((member) => member.agentId === worker.agentId
    || member.workerId === worker.agentId || member.role === worker.role)
    || members[index] || {};
}
