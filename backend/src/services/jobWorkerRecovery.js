...
async function recoverInterruptedJobs(db) {
  ...
  for (const table of ['evaluation_jobs', 'model_jobs']) {
    await db.run(`UPDATE ${table} SET status = CASE WHEN attempts + 1 < max_attempts THEN 'queued' ELSE 'failed' END, attempts = attempts + 1, error_json = ?, completed_at = CASE WHEN attempts + 1 < max_attempts THEN NULL ELSE COALESCE(completed_at, CURRENT_TIMESTAMP) END, claimed_at = NULL, claim_token = NULL, next_attempt_at = NULL WHERE status = 'running' AND (${stale})`, JSON.stringify({ message: 'Worker claim became stale; retry scheduled.', retryable: true, recoveredAt: new Date().toISOString() }), `-${staleMinutes} minutes`);
  }
