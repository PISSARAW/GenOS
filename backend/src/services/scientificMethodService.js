...
function assessFalsification({ hypothesis, predicted, observed, auxiliaryAssumptions = [] } = {}) {
  const hasPrediction = predicted !== undefined && predicted !== null;
  const hasObservation = observed !== undefined && observed !== null;
  const comparable = hasPrediction && hasObservation;
  ...
  return {
    ...
    status: !comparable ? 'undetermined' : matches ? 'survives-test' : 'falsified-under-assumptions',
