...
const SEARCH_STATE = {
  ...
  MEDIUM_VARIATION_STAGNATION: 'medium_variation_stagnation'
}
...
function classifySearchState(agentId, causalReport, entropyMetrics) {
  if (!agentId || !causalReport) {
    return { state: SEARCH_STATE.MECHANICAL_STAGNATION, diagnostics: 'cannot classify, missing inputs' }
  }
  ...
  return {
    ...
    diagnostics: DIAGNOSTICS_BY_REGION[region],
    normalizedEntropy,
    steps,
    searchYield: progress
  }
}
...
const STATE_BY_REGION = {
  ...
  medium_entropy_low_progress: SEARCH_STATE.MEDIUM_VARIATION_STAGNATION,
  inconclusive: SEARCH_STATE.MECHANICAL_STAGNATION
}

const DIAGNOSTICS_BY_REGION = {
  low_entropy_high_progress: { region: 'low_entropy_high_progress', interpretation: 'stable, productive behavior — continue' },
  high_entropy_high_progress: { region: 'high_entropy_high_progress', interpretation: 'variability producing progress — productive exploration' },
  low_entropy_low_progress: { region: 'low_entropy_low_progress', interpretation: 'frozen or repetitive pattern with no evidence gain', cause: 'likely mechanical stagnation or loop' },
  high_entropy_low_progress: { region: 'high_entropy_low_progress', interpretation: 'high variability without progress — panic exploration', cause: 'erratic tool hopping without evidence gain' },
  medium_entropy_low_progress: { region: 'medium_entropy_low_progress', interpretation: 'varied actions with low progress — medium variation stagnation', cause: 'agent explores different paths but none produce evidence' },
  inconclusive: { region: 'inconclusive', interpretation: 'insufficient signal to classify definitively', notes: 'progress and entropy both moderate' }
}
...
module.exports = {
  SEARCH_STATE, classifySearchState, produceSearchReport,
  ENTROPY_LOW_THRESHOLD, ENTROPY_HIGH_THRESHOLD,
  PROGRESS_LOW_THRESHOLD, PROGRESS_MEDIUM_THRESHOLD
}
