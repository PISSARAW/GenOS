...
function memberRecord(source = {}) {
  return {
    ...
    workerId: choose([source.workerId], null),
    role: choose([source.role], 'specialist'),
    domain: choose([source.domain, source.subSystem, source.label], null),
    label: choose([source.label], null),
    subSystem: choose([source.subSystem], null),
    mission: choose([source.mission], null),
    modelTier: choose([source.modelTier], null),
    executionBudgetTokens: Number.isFinite(source.executionBudgetTokens) ? Math.max(0, Math.floor(source.executionBudgetTokens)) : null,
    communicationCadence: choose([source.communicationCadence], null),
    contextHandoff: choose([source.contextHandoff], null),
    capabilities: asList(choose([source.capabilities, source.expertise], [])),
    pipelineStage: Math.max(0, Number(source.pipelineStage) || 0),
...
function teamRunRecord(input = {}) {
  return {
    ...
    execution: input.execution || { planId: input.teamRunId || null, runnerLease: null },
