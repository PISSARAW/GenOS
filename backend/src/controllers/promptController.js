...
async function playground(req, res, next) {
  try { const { prompt = '', models = [], variables = {} } = req.body || {}; const rendered = prompt.replace(/\{\{\s*([\w.-]+)\s*\}\}/g, (_, key) => key.split('.').reduce((value, part) => value == null ? '' : value[part], variables) ?? ''); res.status(202).json({ status: 'queued', models: models.length ? models : ['fake://local'], rendered, results: [] }); } catch (e) { next(e); }
}
module.exports = { listPrompts, createPrompt, getPrompt, createVersion, renderPrompt, playground };
