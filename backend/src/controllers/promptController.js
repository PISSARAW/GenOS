...
async function streamJob(req, res, next) {
  try { const db = await getDatabase(); const job = await db.get('SELECT id FROM model_jobs WHERE id = ?', req.params.id); if (!job) return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Model job not found.' } }); res.setHeader('Content-Type', 'text/event-stream'); res.setHeader('Cache-Control', 'no-cache'); res.setHeader('Connection', 'keep-alive'); let lastId = 0; const send = (event, data) => res.write(`event: ${event}\ndata: ${JSON.stringify(data)}\n\n`); const poll = async () => { const tokens = await db.all('SELECT * FROM model_job_tokens WHERE job_id = ? AND id > ? ORDER BY id', req.params.id, lastId); for (const token of tokens) { lastId = token.id; send('token', token); } const current = await db.get('SELECT status, result_json, error_json FROM model_jobs WHERE id = ?', req.params.id); send('status', current); if (current.status === 'completed' || current.status === 'failed') { send('done', current); clearInterval(timer); res.end(); } }; const timer = setInterval(() => poll().catch(() => {}), 250); req.on('close', () => clearInterval(timer)); await poll(); } catch (e) { next(e); }
}
