...
async function createPrompt(req, res, next) {
  try {
    const db = await getDatabase(); const { name, template = '', variables = [], model = '' } = req.body || {};
    if (!name) return res.status(400).json({ error: { code: 'INVALID_NAME', message: 'Prompt name is required.' } });
    if (!String(model).trim()) return res.status(400).json({ error: { code: 'MODEL_REQUIRED', message: 'A real model URI is required.' } });
...
async function createVersion(req, res, next) {
  try {
    ...
    const { template = '', model = '', config = {} } = req.body || {};
    if (!String(model).trim()) return res.status(400).json({ error: { code: 'MODEL_REQUIRED', message: 'A real model URI is required.' } });
...
async function cancelJob(req, res, next) {
  try {
    ...
    const result = await db.run(
      `UPDATE model_jobs SET status = 'cancelled', error_json = ?, completed_at = CURRENT_TIMESTAMP
       WHERE id = ? AND ${s.clause} AND status IN ('queued', 'running')`,
      JSON.stringify({ message: 'Model job cancelled by operator.', cancelled: true }), req.params.id, ...s.params
    );
...
async function streamJob(req, res, next) {
  try { const db = await getDatabase(); const s = scopeSql(req); const job = await db.get(`SELECT id FROM model_jobs WHERE id = ? AND ${s.clause}`, req.params.id, ...s.params); if (!job) return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Model job not found.' } }); res.setHeader('Content-Type', 'text/event-stream'); res.setHeader('Cache-Control', 'no-cache'); res.setHeader('Connection', 'keep-alive'); let lastId = 0; let polling = false; const send = (event, data) => res.write(`event: ${event}\ndata: ${JSON.stringify(data)}\n\n`); const poll = async () => { if (polling) return; polling = true; try { const tokens = await db.all('SELECT * FROM model_job_tokens WHERE job_id = ? AND id > ? ORDER BY id', req.params.id, lastId); for (const token of tokens) { lastId = token.id; send('token', token); } const current = await db.get(`SELECT status, result_json, error_json FROM model_jobs WHERE id = ? AND ${s.clause}`, req.params.id, ...s.params); send('status', current); if (['completed', 'failed', 'cancelled'].includes(current.status)) { send('done', current); clearInterval(timer); res.end(); } } finally { polling = false; } }; const timer = setInterval(() => poll().catch(() => {}), 250); req.on('close', () => clearInterval(timer)); await poll(); } catch (e) { next(e); }
}
module.exports = { listPrompts, createPrompt, getPrompt, createVersion, renderPrompt, playground, listJobs, cancelJob, streamJob };
