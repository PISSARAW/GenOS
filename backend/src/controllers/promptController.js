...
async function playground(req, res, next) {
  try { const { prompt = '', models = [], variables = {}, config = {} } = req.body || {}; const selectedModels = models.map((model) => String(model).trim()).filter(Boolean); if (!selectedModels.length && !config.modelRouting) return res.status(400).json({ error: { code: 'MODEL_REQUIRED', message: 'Select a real model URI or provide a model routing policy.' } }); const rendered = prompt.replace(/\{\{\s*([\w.-]+)\s*\}\}/g, (_, key) => key.split('.').reduce((value, part) => value == null ? '' : value[part], variables) ?? ''); const db = await getDatabase(); const id = `model-${crypto.randomUUID()}`; const s = scopeSql(req); await db.run('INSERT INTO model_jobs(id,prompt,models_json,config_json,max_attempts,timeout_ms,organization_id,project_id) VALUES(?,?,?,?,?,?,?,?)', id, rendered, JSON.stringify(selectedModels), JSON.stringify(config), Number(config.maxAttempts || 3), Number(config.timeoutMs || 30000), ...s.params); res.status(202).json({ id, status: 'queued', models: selectedModels, rendered }); } catch (e) { next(e); }
}
