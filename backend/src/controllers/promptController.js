...
async function streamJob(req, res, next) {
  try {
    ...
    const job = await db.get(`SELECT id FROM model_jobs WHERE id = ? AND ${s.clause}`, req.params.id, ...s.params);
    if (!job) return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Model job not found.' } });
    ...
    const poll = async () => {
      ...
      polling = true;
      try {
        const tokens = await db.all('SELECT * FROM model_job_tokens WHERE job_id = ? AND id > ? ORDER BY id', req.params.id, lastId);
        for (const token of tokens) {
          lastId = token.id;
          send('token', token);
        }
        const current = await db.get(`SELECT status, result_json, error_json FROM model_jobs WHERE id = ? AND ${s.clause}`, req.params.id, ...s.params);
        if (!current) {
          send('error', { code: 'NOT_FOUND', message: 'Model job not found or removed.' });
          clearInterval(timer);
          res.end();
          return;
        }
