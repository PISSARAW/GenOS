...
async function branchAgentState(req, res) {
  const agentId = String(req.body?.agentId || '').trim();
  const refName = String(req.body?.refName || '').trim();
  const fromCommitId = String(req.body?.fromCommitId || '').trim();
  ...
  await db.run(
    'INSERT INTO agent_state_snapshots (id, agent_id, workspace_id, state_json, reason, commit_message, parent_snapshot_id, ref_name, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    branchId, agentId, agent.workspace_id, source.state_json, 'Agent branch', `Branch ${refName} from ${source.id}`, source.id, refName, req.user?.username || 'agent-operation'
  );
...
async function checkoutAgentState(req, res) {
  ...
  const snapshot = snapshotId
    ? await db.get('SELECT * FROM agent_state_snapshots WHERE id = ? AND agent_id = ?', snapshotId, agentId)
    : await db.get('SELECT * FROM agent_state_snapshots WHERE agent_id = ? AND ref_name = ? ORDER BY created_at DESC, id DESC LIMIT 1', agentId, refName);
  ...
  await db.run(
    ...
    state.dissonance_level || 0, state.eureka_count || 0, state.cognitive_budget ?? 0, state.cognitive_baseline_budget ?? 0,
    state.cognitive_max_dissonance ?? 50, state.is_apoptotic || 0, state.status, state.current_task, agentId
  );
