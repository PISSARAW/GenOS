...
async function replay(req, res) { const db = await getDatabase(); const { incidentId } = req.params; const events = await db.all('SELECT * FROM telemetry_events ORDER BY created_at ASC LIMIT 10000'); const result = safety.buildReplay(incidentId, events, req.body?.stepSpeed); telemetry.emitEvent({ eventType: 'INCIDENT_REPLAY_STARTED', agentId: req.user?.username || 'platform', action: 'REPLAY', detail: `Replay ${incidentId}`, payload: result }); res.json(result); }
...
async function approvals(req, res) { const db = await getDatabase(); const scope = req.tenant; if (req.method === 'GET') return res.json(await db.all('SELECT * FROM platform_approvals WHERE organization_id = ? AND project_id = ? ORDER BY created_at DESC', scope.organizationId, scope.projectId)); const body = req.body || {}; const id = `approval-${Date.now()}-${Math.random().toString(36).slice(2,7)}`; await db.run('INSERT INTO platform_approvals (id,action,agent_id,risk,uncertainty,requested_by,organization_id,project_id,payload_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', id, body.action || 'unknown', body.agentId || null, body.risk || 'high', Number(body.uncertainty || 0), req.user?.username || 'platform', scope.organizationId, scope.projectId, JSON.stringify(body)); res.status(201).json({ id, status: 'pending', ...body }); }
