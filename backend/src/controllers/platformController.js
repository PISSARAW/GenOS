...
const bisection = require('../services/bisectionService');
...
async function providers(req, res) {
  ...
  if (!rows.length) return res.json(safety.DEFAULT_PROVIDERS);
...
async function bisect(req, res) { const db = await getDatabase(); const snapshots = await db.all('SELECT step_number AS step, snapshot_hash AS hash, author AS agent, reason AS desc FROM workspace_snapshots WHERE workspace_id = ? ORDER BY step_number', req.body?.workspaceId || 'ws-genos-core'); res.json(bisection.bisectAnomaly(snapshots, snap => snap.step < Number(req.body?.firstBadStep || Infinity))); }
