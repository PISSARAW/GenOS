...
async function ingestSpan(req, res, next) { try { const db = await getDatabase(); const raw = req.body || {}; const body = raw.framework ? frameworkAdapters.normalize(raw.framework, raw) : raw; const id = body.spanId || body.id || `span-${crypto.randomUUID()}`; const traceId = body.traceId || body.trace_id || `trace-${crypto.randomUUID()}`; await db.run('INSERT OR REPLACE INTO trace_spans (id, trace_id, agent_id, parent_span_id, name, start_time, end_time, inputs_json, outputs_json, error) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', id, traceId, body.agentId || body.agent_id || 'unknown', body.parentSpanId || body.parent_span_id || null, body.name || 'span', Number(body.startTime || body.start_time || Date.now()), body.endTime || body.end_time ? Number(body.endTime || body.end_time) : null, JSON.stringify(body.inputs || body.attributes || {}), JSON.stringify(body.outputs || {}), body.error || null); res.status(201).json({ id, traceId, framework: body.framework || null }); } catch (e) { next(e); } }
