#!/usr/bin/env node
/**
 * GenOS solar-direct runtime.
 * Pattern: identique à local-codex-runtime.cjs (protobuf framing, prompts,
 * events encodés, pipeline post-génération) mais le moteur cognitif est Solar
 * appelé directement via l'API OpenAI-compatible du provider Nous (token OAuth
 * depuis le fichier d'auth Hermes), sans passer par Ollama, Codex ni caller_mcp.
 */
'use strict';
...
function validateGeneration(reply, state) {
  ...
  if (state.tokensUsed > budgetLimit(state.executionBudget, 'tokens')) {
    throw new Error(`Solar generation exceeded token budget (${state.tokensUsed} > ${budgetLimit(state.executionBudget, 'tokens')}).`);
  }
}

function guardCompletionBudget(state) {
  if (state.eventCount + 1 > budgetLimit(state.executionBudget, 'events')) {
    throw new Error(`Solar runtime exceeded event budget (${state.eventCount + 1} > ${budgetLimit(state.executionBudget, 'events')}).`);
  }
}
...
function emitCompletion(state, reply) {
  ...
  emitEvent(state, {
    ...
    status: 'completed',
    payload: { evidenceReport: report, usage: state.usage }
  });
}
