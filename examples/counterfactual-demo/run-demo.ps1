...
try {
	...
	Write-Host "[1/5] init + create agent A"
	Invoke-Genos init
	Invoke-Genos agent create --name clone-no-llm --role tester --out $agentPath --format json

	Write-Host "[2/5] create snapshot S0 with a minimal seeded memory"
	...
	Write-Host "[3/5] fork A1 and A2 from S0 (no LLM call, no JSON editing)"
	...
	Write-Host "[4/5] assert same logical state and distinct identity"
	...
	Write-Host "[5/5] assert isolated event streams via replay"
