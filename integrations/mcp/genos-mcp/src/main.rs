use clap::{Parser, Subcommand};
use genos_mcp::{authenticated_http_router, serve_stdio, GenosCliExecutor, McpServer};
use std::{net::SocketAddr, sync::Arc};

#[derive(Debug, Parser)]
#[command(
    name = "genos-mcp",
    about = "Expose GenOS lifecycle operations over MCP"
)]
struct Cli {
    #[command(subcommand)]
    transport: Option<Transport>,
}

#[derive(Debug, Subcommand)]
enum Transport {
    /// Serve newline-delimited MCP JSON-RPC over stdin/stdout (default).
    Stdio,
    /// Serve stateless MCP JSON-RPC over Streamable HTTP.
    Http {
        #[arg(long, default_value = "127.0.0.1:8799")]
        bind: SocketAddr,
    },
}

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    let cli = Cli::parse();
    let server = McpServer::new(Arc::new(GenosCliExecutor::discover()?));
    match cli.transport.unwrap_or(Transport::Stdio) {
        Transport::Stdio => serve_stdio(tokio::io::stdin(), tokio::io::stdout(), server).await,
        Transport::Http { bind } => {
            let token = std::env::var("GENOS_MCP_TOKEN")
                .map_err(|_| anyhow::anyhow!("GENOS_MCP_TOKEN is required for HTTP transport"))?;
            if token.trim().is_empty() {
                anyhow::bail!("GENOS_MCP_TOKEN must not be empty for HTTP transport");
            }
            let listener = tokio::net::TcpListener::bind(bind).await?;
            eprintln!("genos-mcp listening on http://{bind}/mcp");
            axum::serve(listener, authenticated_http_router(server, token)).await?;
            Ok(())
        }
    }
}
