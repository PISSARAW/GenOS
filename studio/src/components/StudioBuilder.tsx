...
export const StudioBuilder: React.FC = () => {
  ...
  const persistWorkflow = async () => {
    ...
    } catch (cause: any) { setSaveState('offline'); setError(messageOf(cause, 'Workflow could not be saved.')); return null; }
  };
  ...
  return <div className="studio-builder">
    ...
    {tab === 'build' && <BuildView nodes={nodes} edges={edges} onNodesChange={handleNodesChange} onEdgesChange={handleEdgesChange} onConnect={onConnect} selectedNode={selectedNode} setSelectedNode={setSelectedNode} addWorkflowNode={addWorkflowNode} updateNodeLabel={updateNodeLabel} onDeleteNode={deleteNode} />}
    {tab === 'prompts' && <PromptLab />}{tab === 'runs' && <RunsView />}{tab === 'evals' && <EvalsView />}{tab === 'rag' && <RagView />}{tab === 'integrations' && <IntegrationsView />}{tab === 'deploy' && <DeployView workflowId={workflowId} />}
  </div>;
};
...
function PromptLab() {
  const [prompts, setPrompts] = useState<any[]>([]); const [selectedId, setSelectedId] = useState(''); const [version, setVersion] = useState(1); const [name, setName] = useState(''); const [template, setTemplate] = useState(''); const [models, setModels] = useState('local://runtime'); const [variables, setVariables] = useState('{}'); const [result, setResult] = useState<any>(null); const [streamText, setStreamText] = useState(''); const [status, setStatus] = useState(''); const [error, setError] = useState(''); const [loading, setLoading] = useState(true); const [action, setAction] = useState('');
...
function DeployView({ workflowId }: { workflowId: string | null }) {
  ...
  const deploy = async () => { setAction('Deploying…'); setResult(''); try { const response = await api.deployAgent({ prompt: prompt.trim(), agentType: 'GenOS', modelTier: 'flash' }); setResult(`Deployment accepted: ${response.agent?.id || response.agentId || 'backend response received'}`); await load(); } catch (cause: any) { setResult(messageOf(cause, 'Deployment failed.')); } finally { setAction(''); } };
