...
export const SwarmTopologyGraph: React.FC<SwarmTopologyGraphProps> = ({ onSelectAgent }) => {
  ...
  useEffect(() => {
    ...
    refreshRef.current = () => { if (mounted) refresh(); };
    refresh();
    ...
    <div style={{ background: 'var(--bg-panel)', border: '1px solid var(--panel-border)', borderRadius: '6px', overflow: 'hidden', height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '10px 16px', background: 'var(--bg-subtle)', borderBottom: '1px solid var(--panel-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div
          ...
          style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '0.75rem', color: error ? 'var(--danger)' : 'var(--success)', cursor: error ? 'pointer' : 'default' }}
        >
          {error ? <RefreshCw size={12} /> : <Activity size={12} className="pulse-green" />}
          {error ? 'Disconnected · click to retry' : `${topology?.nodeCount || 0} nodes · ${topology?.edgeCount || 0} links`}
        </div>
      </div>
      <div style={{ padding: '6px 16px', color: 'var(--text-secondary)', fontSize: '0.7rem', borderBottom: '1px solid var(--panel-border)', display: 'flex', justifyContent: 'space-between' }}>
        <span>
          Backend snapshot: {lastUpdate} · {liveParticles.length} recent message{liveParticles.length === 1 ? '' : 's'}
        </span>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '12px' }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
            <svg width="22" height="6"><line x1="0" y1="3" x2="22" y2="3" stroke="var(--panel-border)" strokeWidth="1.5" /></svg>
            telemetry link
          </span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
            <svg width="22" height="6"><line x1="0" y1="3" x2="22" y2="3" stroke="var(--panel-border)" strokeWidth="1.5" strokeDasharray="4 2" /></svg>
            lineage / fleet / workspace
          </span>
        </span>
      </div>
      <div style={{ flex: 1, position: 'relative', minHeight: '380px', background: 'var(--bg-main)' }}>
        <svg width="100%" height="100%" viewBox="0 0 600 420" style={{ display: 'block' }}>
          {edges.map((edge, index) => {
            ...
            return (
              <line key={`${edge.from}-${edge.to}-${index}`} x1={from.x} y1={from.y} x2={to.x} y2={to.y} stroke="var(--panel-border)" strokeWidth="1.5" strokeDasharray={edge.type === 'telemetry' ? undefined : '4 2'}>
                <title>{`${edge.type || 'link'} edge: ${from.label} → ${to.label}`}</title>
              </line>
            );
          })}
          ...
          {nodes.map((node) => (
            ...
            >
              <title>{`${node.label}${node.role ? ` (${node.role})` : ''} — ${node.status}${onSelectAgent ? ' · click to inspect' : ''}`}</title>
