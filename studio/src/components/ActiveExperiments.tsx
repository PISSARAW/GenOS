...
import {
  FlaskConical, Target, Octagon, User
} from 'lucide-react';
import { api } from '../api/client';
import { useToastStore } from '../store/useToastStore';

const FlatSparkline: React.FC<{ data: any[]; color: string }> = ({ data, color }) => {
  const values = data
    .map((d) => (typeof d === 'number' ? d : Number(d?.successRate ?? d?.value)))
...
export const ActiveExperiments: React.FC<{ onOpenLab?: (experimentId?: string) => void }> = ({ onOpenLab }) => {
  ...
  const handleHaltNonCritical = async () => {
    if (!window.confirm('Engage the global MCP kill switch? All running agents and experiments will be halted.')) return;
    try {
      await api.haltAll();
  ...
  const mergedExperiments = backendExperiments.map((be) => ({
      id: be.id,
      title: be.title,
      type: be.type || 'Uncategorized',
      observations: Array.isArray(be.data) ? be.data.length : 0,
      target: Number(be.target),
      color: be.color || '#58a6ff',
      data: Array.isArray(be.data) ? be.data : []
    }));

  const filters = ['All', ...Array.from(new Set(mergedExperiments.map((exp) => exp.type)))];
  ...
  const successObservations = mergedExperiments
    .flatMap((exp) => exp.data)
    .map((d: any) => Number(d?.successRate))
    .filter((n) => Number.isFinite(n));
  const successRate = successObservations.length > 0
    ? successObservations.reduce((sum, n) => sum + n, 0) / successObservations.length
    : null;
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '32px' }}>
              ...
              {successRate !== null && (
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Target size={14} /> Success Rate (observed)
                  </span>
                  <span style={{ fontSize: '1.5rem', fontWeight: 600, color: 'var(--text-secondary)' }}>{successRate.toFixed(1)}%</span>
                </div>
              )}
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '24px' }}>
            ...
            {filteredExperiments.map((exp) => {
              const hasTarget = Number.isFinite(exp.target) && exp.target > 0;
              const percent = hasTarget ? Math.min((exp.observations / exp.target) * 100, 100) : null;
              return (
                <div
                  key={exp.id}
                  onClick={() => onOpenLab?.(exp.id)}
                ...
                >
                  ...
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: '6px', fontWeight: 600 }}>
                      <span>{exp.observations} observations</span>
                      <span style={{ color: percent !== null ? 'var(--text-primary)' : 'var(--text-muted)' }}>
                        {percent !== null ? `${Math.round(percent)}%` : 'target unknown'}
                      </span>
                    </div>
                    <div style={{ width: '100%', height: '4px', background: 'var(--bg-main)', border: '1px solid var(--panel-border)', borderRadius: '2px', overflow: 'hidden' }}>
                      <div style={{ width: percent !== null ? `${percent}%` : '40%', height: '100%', background: exp.color, opacity: percent !== null ? 1 : 0.4 }}></div>
                    </div>
                  </div>
