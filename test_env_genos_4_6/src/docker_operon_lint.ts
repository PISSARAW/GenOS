...
lines.forEach((line, idx) => {
  ...
  if (instr === 'FROM') { sawFrom = true; }
  if (instr === 'WORKDIR') { sawWorkdir = true; }
  if ((instr === 'COPY' || instr === 'ADD') && !sawWorkdir && sawFrom) {
    diagnostics.push(`ligne ${idx + 1}: COPY/ADD sans WORKDIR préalable`);
  }
  if (instr === 'USER' && /node/i.test(line)) { sawUserNode = true; }
...
if (diagnostics.length === 0) { console.log('DOCKER LINT: OK (FROM premier, WORKDIR avant COPY, USER node, HEALTHCHECK présent)'); }
else { console.log(`DOCKER LINT: ${diagnostics.length} problème(s)`); diagnostics.forEach((d) => console.log(`  - ${d}`)); process.exitCode = 1; }
