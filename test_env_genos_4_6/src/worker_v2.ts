...
interface Locus { gene_name: string; value: number; epigenetic_marker: number; }
const genome = readFileSync('api_worker.yaml', 'utf8');
...
function marker(name: string): number {
...
for (let p = 1; p <= 12; p += 1) {
  ...
  try {
    callUnstableApi(obscureToken(Math.floor(Date.now() / 400)), `w2-${p}`);
    successes += 1;
  } catch (err) {
