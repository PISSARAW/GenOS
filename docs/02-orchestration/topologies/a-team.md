...
|---|---|---|
| Expert Committee | Matrice d'expertises, recouvrements/conflits, quorum et procédure explicite de consensus, dissidence et calibration. Le service d'exécution accepte les votes avec preuves et calcule le résultat/dissensus. | L'indépendance est imposée par le contrat de contexte initial; la calibration est requise mais aucun score calibré n'est inventé. Les votes doivent être soumis au service d'exécution par l'intégrateur. |
| Pipeline | Les handoffs appliquent les schémas JSON d'entrée/sortie; `executePipeline` fournit retries locaux bornés, cache par stage, reprise et flux séquentiel avec backpressure. | Le runner est exposé comme service et n'est pas automatiquement appelé par tous les adaptateurs de workers. |
| Project DAG | Le WorkGraph ordonne les dépendances; la politique calcule chemin critique, ordonnancement sous capacités, jointures conditionnelles et fermeture transitive des invalidations; handoffs typés valident les schémas. | L'ordonnancement ressource/chemin critique est un plan calculé au dispatch; la capacité runtime du WorkGraph reste l'ordonnanceur effectif. La réexécution minimale doit consommer `invalidatedNodes`. |
| Cross-Functional Pod | Limite les dépendances externes déclarées, exige un propriétaire unique par artefact, produit les contrats et score d'autonomie. | Le score mesure les déclarations; il ne prouve pas l'autonomie réelle ni l'achèvement automatique de design→build→test→ship. |
| Boundary-Spanner | Contrats sémantiques versionnés exigent schéma, provenance, règles de traduction si nécessaire et vérification des schémas d'extrémité déclarés. | La détection de dérive et la double validation sont des contrôles à appeler lors des échanges; aucun capteur continu de dérive n'est démarré au dispatch. |
| Matrix Team | Autorités uniques par type de décision; `resolveMatrixDecision` applique contrôle des deux propriétaires et compare-and-swap de révision. | Le service est une opération pure à connecter au stockage transactionnel du consommateur; les ACL ne sont pas modifiées par le dispatch. |
| Tiger Team | Mandat limité, timebox dure, liste d'actions autorisées/interdites, budget et limites de ressources; le garde runtime refuse une action hors mandat ou expirée. | La persistance d'audit, révocation effective des privilèges et création automatique du post-mortem relèvent de l'adaptateur d'exécution. |
| Incident Command | Rôles ICS requis, cadence SITREP/objectifs, span-of-control vérifié par responsable, timeline et transitions de handover/clôture. | Les événements de période sont réduits par le service runtime; l'ordonnanceur ne déclenche pas encore automatiquement les SITREP selon cadence. |
| Multiteam System | Composition programme vérifie les contrats et le DAG, matérialise conseil/objectifs et budgets locaux; le runner persiste les équipes et bloque sur contrat ou dépassement budgétaire. | La récursion team-of-teams est bornée mais la composition imbriquée reste à intégrer; l'exécution nécessite un adaptateur `executeTeam` vérifié. |
| Adaptive A-Team | Compare gaps de capacités; `authorizeStaffingChange` bloque sans preuve, hystérésis, budget, stabilité ou quota de changements. Les phases restent planifiables. | Le contrôle autorise/refuse une proposition mais n'exécute pas recrutement, libération, transfert mémoire ou transaction Morphogenèse; aucune boucle de staffing périodique n'est activée. |
| Relay Team | Paquet séquentiel canonique haché, limite de résumé, accusé du receveur et décision de rollback vers le propriétaire précédent. | L'accusé/rollback sont des opérations de service; le lease de propriété décrit dans le plan doit être branché sur un stockage à révision CAS pour garantir l'exclusivité entre processus. |
...
Implémentation : [`operationalVariantPolicy.js`](../../../backend/src/services/aTeam/variants/operationalVariantPolicy.js), appelée par [`dispatchPolicyService.js`](../../../backend/src/services/aTeam/dispatchPolicyService.js), et opérations runtime dans [`variantExecutionService.js`](../../../backend/src/services/aTeam/variants/variantExecutionService.js). Les tests sont dans `test_ateam_dispatch_policy.js` et `test_ateam_variant_execution.js`. Une opération pure ne constitue pas une permission persistée : les adaptateurs doivent conserver les contrôles de preuve, d'autorité et de révision.
