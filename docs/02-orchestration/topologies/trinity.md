...
Quand plusieurs mondes non dominés restent, `SYNTHESIZE_CLAIMS` est retourné si les claims reliés par `supports`, `verifies` ou `complements` possèdent chacun des références d'évidence valides et qu'aucune arête `contradicts` mission n'unit ces claims. Les claims synthétisés et le graphe sont persistés. Cette décision ne promeut pas un artefact : faute d'assembleur de workspace synthétique en v1, `promotion` reste refusée avec `synthesized_claims_require_artifact_assembly`.
