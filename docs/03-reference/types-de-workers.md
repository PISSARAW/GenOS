...
- **Désactivé** : spawn/délégation imbriqués (`spawn:false/delegate:false/budget:0/depth:0` durcis, `assertRuntimeContract` refuse, `subOrchestratorService` non raccordé, baux mémoire seuls).
- **Asymétries connues** : `PROMPT_RULES.sub_orchestrator` mentionne des plafonds à `0` ; `orchestratorDispatchService` n'injecte pas `evidenceRule` ; projections `procedural/symbiotic/formal` sur `BoundedWorker` ; `teaching_worker` sur `ScoutCell`.
...
| `orchestratorDispatchService.buildWorkerMission` | `resolve + build`, injection `promptRule` seul. |
| `agentRuntimeAdapter/missionBootstrap.resolveWorkerIdentity` | Verrou boot : `MISMATCH` + `UNSUPPORTED_DELEGATION`. |
...
| `agents/subOrchestratorService.js` | Non appelé par les chemins ci-dessus ; baux mémoire seuls. |
