...
|---|---|---|---|
| GET | `/api/security/csrf` | — | Issue an anti-CSRF token. |
...
| GET | `/api/workspaces/:id/files` | — | Workspace file tree. |
| GET | `/api/workspaces/diff` | — | Diff two references. |
| POST | `/api/workspaces/bisect` | workspace:write | Causal bisection over snapshots. |
| GET | `/api/workspaces/:id/snapshots` | — | Snapshot history. |
...
| GET | `/api/workspaces/:id/rollback-preview` | — | Preview an atomic rollback step. |
...
| GET | `/api/deploy/trinity` | — | List trinity worlds. |
| POST | `/api/deploy/trinity` | workspace:write | Launch a trinity workflow. |
| GET | `/api/agents` | — | List agents (paginated). |
| GET | `/api/agents/history` | — | Historical runs. |
| GET | `/api/agents/:id/dossier` | — | Full dossier for one agent. |
| POST | `/api/agents/spawn` | workspace:write | Spawn an agent. |
| POST | `/api/agents/:id/start` · `/stop` | workspace:write | Start/stop an agent. |
| DELETE | `/api/agents/:id` | workspace:write | Delete an agent. |
| POST | `/api/agents/bulk-stop` · `/bulk-delete` | workspace:write | Fleet operations. |
| POST | `/api/agents/:id/ping` | — | Liveness ping. |
| POST | `/api/agents/:id/subscribe` | — | Subscribe to agent events. |
| POST | `/api/agents/:id/events` | workspace:write | Ingest an agent event. |
| GET | `/api/agents/:id/workers/garage` | — | Worker roster of an orchestrator. |
...
| GET | `/api/agents/:id/execution-runs` (+`/latest`) | — | Execution run history. |
...
| POST | `/api/workflows/:id/validate` | — | Validate a graph. |
...
| POST | `/api/prompts/:id/render` | — | Render a version with variables. |
...
| Method | Path | Permission | Description |
|---|---|---|---|
| GET | `/api/traces` · `/api/traces/:traceId` | — | Trace explorer. |
| POST | `/api/traces/ingest` | — | Ingest a span. |
| POST | `/api/traces/:traceId/replay` | — | Causal replay of a trace. |
| GET | `/api/telemetry/events` · `/dashboard` · `/achievements` | telemetry:read | Telemetry queries. |
...
| GET | `/api/status` · `/api/health` · `/api/dashboard` | mixed | Runtime status endpoints. |
| GET | `/api/platform/causal-graph` · `/telemetry/summary` | — | Platform observability. |
...
| GET | `/api/experiments/recent` · `/analysis` · `/thoughts` · `/coevolution` · `/:id/waves` | — | Experiment views. |
| GET | `/api/evals/datasets` (+`/:id/cases`, `/jobs`) | — | Dataset and job registry. |
| POST | `/api/evals/datasets` (+cases, `/jobs`) | write / experiment:run | Tenant-scoped evaluation writes. |
| GET | `/api/evaluation/overview` | — | MCTS and evaluation state. |
...
| GET/POST | `/api/arena/tournament` · `/pareto` · `/trace` | — | Solver tournaments and Pareto views. |

## Swarm, lineage and memory

| Method | Path | Permission | Description |
|---|---|---|---|
| GET | `/api/swarm/consensus` · `/metrics` · `/topology` | — | Swarm monitor. |
| POST | `/api/swarm/proposals` | swarm:propose | Create a proposal. |
| POST | `/api/swarm/vote` | swarm:vote | Cast a vote. |
| GET | `/api/lineage` · `/genome/graph` · `/phylogeny` · `/alleles` | — | Genome and lineage graphs. |
| POST | `/api/nodes/clone` · `/nodes/kill` | workspace:write | Node operations. |
| POST | `/api/genome/crossover` · `/synthesize` · `/decision` | workspace:write | Genetic operations. |
| GET/POST | `/api/memory/search` | — | Experience search. |
| POST | `/api/memory/cherry-pick` · `/counterfactual` | — | Memory transfer operations. |
...
| POST | `/api/resilience/cryptobiosis/freeze` · `/thaw` | emergency_kill | Freeze/thaw agents. |
| GET/POST | `/api/resilience/drift` | — | Drift analysis. |
| GET | `/api/alerts` · `/api/incidents` | — | Incident views (alerts paginated). |
| POST | `/api/incidents/replay` · `/api/platform/incidents/bisect` | write | Replay and bisection. |
...
| Method | Path | Permission | Description |
|---|---|---|---|
| GET | `/api/config` · `/api/model` · `/api/model/local` | — | Configuration and model discovery. |
...
| GET/POST | `/api/platform/permissions` · `/audit` · `/approvals` | security:manage | Governance surfaces. |
...
| POST | `/api/platform/evaluations/pareto` | — | Pareto frontier computation. |

## MCP tools

| Method | Path | Permission | Description |
|---|---|---|---|
| GET | `/api/tools` · `/tools/metrics` · `/tools/:name/schema` | — | Tool catalogue. |
| POST | `/api/tools/dry-run` · `/tools/test` | safe test | Simulate or execute safely. |
| POST | `/api/mcp/execute` · `/mcp/equip` | mcp:execute_safe / write | Execute/equip a tool. |
...
| Method | Path | Permission | Description |
|---|---|---|---|
| GET | `/api/rust/status` · `/snapshots` | read | CLI availability and snapshot list. |
| POST | `/api/rust/snapshots` | workspace:write | Create a snapshot through the core. |
| POST | `/api/rust/diff` · `/replay` · `/hallucination/*` | read / experiment:run | Core-backed diff, replay, hallucination ops. |

## Product proofs

| Method | Path | Permission | Description |
|---|---|---|---|
| GET | `/api/product-proofs/safe-debugging` | — | Latest proof evidence. |
...
| GET | `/environments` · `/workers` | — | Environments and worker pool status. |
...
| Method | Path | Permission | Description |
|---|---|---|---|
| GET | `/api/registry` · `/:kind` · `/marketplace` | — | Artifact registry. |
...
| GET/POST | `/api/releases/rollouts` (+metrics, decide) · `/chargeback` | write on POST | Progressive rollouts. |
...
| GET | `/api/compliance/frameworks` · `/reports` | — | Compliance reports. |
| POST | `/api/compliance/reports` | workspace:write | Generate a report. |
| GET | `/api/compliance/reports/:id/export` | — | Export a report. |
| GET | `/api/ide/contract` · `/integrations` | — | IDE contract and connections. |
| POST | `/api/ide/integrations` · `/commands/:command` | — | IDE command dispatch. |
...
| GET | `/api/schema/status` · POST `/schema/migrate` | write | Migration status. |
...
| Method | Path | Permission | Description |
|---|---|---|---|
| POST | `/api/command` · `/api/terminal` | admin/operator | Command dispatch and terminal. |
| GET | `/api/trajectories` (+`/pending`, `/active`) | — | Trajectory review queues. |
