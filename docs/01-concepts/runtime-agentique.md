...
Le bus de signalisation décrit ci-dessous est un modèle et un ensemble de composants, pas une garantie de livraison entre instances. `biomimeticSignalingBus.js` encode et évalue des signaux ; `signalingTransportService.js` sert aux outils explicites `genos_signal_*` et aux services de décision collective, et maintient aussi un journal `Map` limité au processus Node courant. Les messages de l'organisation et les signaux persistés dans SQLite peuvent être relus par les processus qui partagent la base, mais aucun mécanisme ne les notifie et aucune livraison n'est garantie. Le dispatch MCP n'impose pas ce transport aux handlers biomimétiques : stigmergie et assimilation plasmidique ne publient dans l'inbox d'organisation que lorsque l'appel fournit `orchestrator_id` (et l'identifiant d'agent requis).

Le modèle vise à réduire l'overhead des invites textuelles en langage naturel et des payloads JSON entre agents autonomes ([backend/src/services/biomimeticSignalingBus.js](../../backend/src/services/biomimeticSignalingBus.js) et [backend/src/services/dynamicOrganizationService.js](../../backend/src/services/dynamicOrganizationService.js)) :
...
5. **Frontière d'Incompressibilité (cible du modèle)** : Le texte peut être remplacé par des signaux compacts dans les composants qui les prennent en charge. La couche de transport persistante utilise actuellement `signal_blobs` et `signal_subs` ; elle ne garantit pas la circulation de tous les états intermédiaires entre instances.
