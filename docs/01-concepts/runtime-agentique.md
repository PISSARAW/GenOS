...
Le bus de signalisation décrit ci-dessous est un modèle et un ensemble de composants, pas une garantie de communication active entre instances. `biomimeticSignalingBus.js` encode et évalue des signaux ; `signalingTransportService.js` les persiste dans SQLite et maintient aussi un journal `Map` limité au processus Node courant. Une autre instance ayant accès à la même base peut relire les lignes persistées, mais aucun mécanisme ne la notifie ni ne garantit leur livraison. Le transport n'est pas intégré nativement aux handlers biomimétiques existants. La portée de diffusion reste donc locale au processus backend.
