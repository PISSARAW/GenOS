...
fn step_flat(grid: &[u32; 20], next: &mut [u32; 20]) {
    for y in 0..20 {
        ...
        add(ym << 1); add(ym); add(ym >> 1);
        add(yc << 1);          add(yc >> 1);
        add(yp << 1); add(yp); add(yp >> 1);
...
fn sa_thread(tid: usize, ctx: Context) {
    ...
    let mut chains = [State { grid: [0; 20], score: 0, errors: [0; 20] }; 64];
    
    for c in &mut chains {
        for y in 0..20 { c.grid[y] = rng.gen_range(0..=0xFFFFF); }
    ...
    while start.elapsed().as_secs_f64() < 45.0 {
        iter += 1;
        if best.score == 400 { break; }

        let cid = iter % 64;
        let mut current = chains[cid];

        if iter % 100_000 == 0 {
            current.grid = crossover(&best, ctx.shared);
            evaluate(&current.grid.clone(), ctx.target, &mut current);
            if current.score > best.score {
                best = current;
                last_best_iter[cid] = iter;
            }
            if let Ok(mut g) = ctx.shared.try_lock() { g[tid] = best; }
            chains[cid] = current;
            continue;
        }
        ...
        if delta > 0.0 || rng.gen_bool(p.clamp(0.0, 1.0)) {
            current = next;
            if current.score > best.score {
                best = current;
                last_best_iter[cid] = iter;
                if best.score >= 370 {
                    println!("[Thread {} Chain {}] New Best: {}/400 (iter {})", tid, cid, best.score, iter);
                }
            } else if current.score == 378 && iter - last_best_iter[cid] > 20_000 {
    ...
    if let Ok(mut g) = ctx.shared.lock() {
        if best.score > g[tid].score { g[tid] = best; }
    }
}
...
fn main() {
    ...
    if global_best == 400 {
        ...
        println!("Bloqué à {}. Le Dieu Darwinien doit encore évoluer...", global_best);
    }
}
