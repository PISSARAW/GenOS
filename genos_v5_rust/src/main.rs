...
static mut NEIGHBORS_LIST: [Neighbors; 400] = [Neighbors { list: [0; 8], len: 0 }; 400];
...
fn init_neighbors() {
    unsafe {
        for i in 0..400 {
            ...
            NEIGHBORS_LIST[i] = Neighbors { list, len };
        }
    }
}

fn load_target() {
    let file = File::open("../target_grid.txt").unwrap();
...
fn get_next_cell(c: i8, c1: i8, c2: i8) -> i8 {
    ...
    let can_d = if c == 2 { !(c1 == 3 && sum_max == 3) } else { c1 < limit || sum_max > 3 };
    if can_a && can_d { 2 } else if can_a { 1 } else { 0 }
}
...
fn check_prune(proj: &[i8; 400]) -> bool {
    unsafe {
        for i in 0..400 {
            let p = proj[i];
            if p != 2 && p != TARGET_GRID[i] { return true; }
        }
    }
    false
}
...
fn project_real_and_get_hamming(grid: &[i8; 400]) -> i32 {
    let mut curr = [0; 400];
    for i in 0..400 {
        curr[i] = if grid[i] == 2 { 0 } else { grid[i] };
    }
    let mut next = [0; 400];
    unsafe {
        for _ in 0..5 {
            for i in 0..400 {
                ...
                let mut ones = 0;
                for j in 0..n_list.len {
                    if curr[n_list.list[j]] == 1 { ones += 1; }
                }
                if curr[i] == 1 {
        ...
        let mut dist = 0;
        for i in 0..400 {
            if curr[i] != TARGET_GRID[i] { dist += 1; }
        }
        dist
    }
}
...
#[derive(Clone, Copy)]
struct VarOrder { i: usize, dist: f64 }
...
fn solve_dfs(grid: &mut [i8; 400], var_idx: usize, vars_order: &[usize; 400], start_time: Instant) -> bool {
    unsafe {
        CALLS += 1;
        ...
        if real_dist < BEST_DIST {
            BEST_DIST = real_dist;
            BEST_GRID = *grid;
            if BEST_DIST == 0 && var_idx == 400 { return true; }
        }
        
        if var_idx == 400 { return false; }
        
        let idx = vars_order[var_idx];
        
        grid[idx] = 0;
        if solve_dfs(grid, var_idx + 1, vars_order, start_time) { return true; }
...
fn main() {
    ...
    unsafe {
        ...
        let mut file = File::create("../gen0_v5_rust.txt").unwrap();
        for y in 0..20 {
