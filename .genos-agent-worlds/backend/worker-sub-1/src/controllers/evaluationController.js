...
exports.pruneNode = async (req, res, next) => { try { const result = await service.pruneNode(req.params.id, req.tenant); if (!result) return res.status(404).json({ error: { code: 'MCTS_NODE_NOT_FOUND', message: 'MCTS node not found' } }); res.json(result); } catch (e) { next(e); } };
